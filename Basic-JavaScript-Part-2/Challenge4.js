...
for (let i = 0; i < bills.length; i++) {
    tips.push((bills[i] >= 50 && bills[i] <= 300) ? 0.15 * bills[i] : (bills[i] < 50) ? bills[i] * 0 : bills[i] * 0.2);
...
console.log(`Average of bills id ${calcAvg(bills)}`);
console.log(`Average of tips id ${calcAvg(tips)}`);
console.log(`Average of total id ${calcAvg(total)}`);